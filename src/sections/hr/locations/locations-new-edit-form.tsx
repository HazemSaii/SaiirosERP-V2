...
const LocationNewEditForm = forwardRef<LocationNewEditFormHandle, Props>(
  ({ currentLocation, operation, countries, countriesLoading }, ref) => {
    ...
    const NewApprovalSchema = z.object({
      locationName: z.object({
        AR: z
          .string()
          .min(1, { message: t('Location Name is required') })
          .regex(/^[A-Za-z\u0600-\u06FF][A-Za-z\u0600-\u06FF0-9\s]{2}[A-Za-z\u0600-\u06FF0-9\s@#$%^&*()]*$/, {
            message: t('Invalid Location Name'),
          })
          .refine((val) => val.trim().length > 0, { message: t('Location Name is required') })
          .optional(), // Make it optional if not needed
    
        EN: z
          .string()
          .min(1, { message: t('Location Name is required') })
          .regex(/^[A-Za-z0-9\s@#$%^&*()]*$/, { message: t('Invalid Location Name') })
          .refine((val) => val.trim().length > 0, { message: t('Location Name is required') })
          .optional(),
      })
      .superRefine((data, ctx) => {
        if (currentLanguage === 'AR' && !data.AR) {
          ctx.addIssue({
            ...
            message: t('Location Name is required in Arabic'),
            path: ['AR'],
          });
        }
        if (currentLanguage === 'EN' && !data.EN) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: t('Location Name is required in English'),
            path: ['EN'],
          });
        }
        ...
        addressLine2: z
          ...
          .refine((value) => {
            ...
            return /^[A-Za-z0-9\u0600-\u06FF][A-Za-z0-9\u0600-\u06FF\s]{2}[A-Za-z0-9\u0600-\u06FF\s@#$%^&*()]*$/.test(value);
    ...
    //   }
    // };
    const validateFieldOnBlur = useMemo(() => async (name: string) => {
      try {
        return await trigger(name);
      } catch (error) {
        console.error(`Validation error on ${name}:`, error);
        ...
        >
          ...
          <Box sx={{ gridColumn: { xs: 'span 4', sm: 'span 2', md: 'span 2' } }}>
            ...
            required
              ...
              options={!countriesLoading ? countries.map((option: any) => option.countryCode) : []}
