...
export default function PositionsTableRow({ row, onEditRow, onDeleteRow, deleteLoading }: Props) {
  const {
    positionName,
    organizationName,
    jobName,
  ...
  return (
    <>
      <TableRow hover>
        ...
        >
          <Avatar alt={positionName} src={`${IMAGE_BASE}/${avatarUrl}`} sx={{ mr: 2 }} />
          <ListItemText primary={positionName} primaryTypographyProps={{ typography: 'body2' }} />
        </TableCell>

        <TableCell sx={{ whiteSpace: 'nowrap' }}>{organizationName}</TableCell>
        <TableCell sx={{ whiteSpace: 'nowrap' }}>{jobName}</TableCell>
        <TableCell sx={{ whiteSpace: 'nowrap' }}>{headcounts}</TableCell>
        <TableCell>
        ...
        <TableCell>
          <Label variant="soft" color={activeColor}>
            {active}
          </Label>
        </TableCell>
