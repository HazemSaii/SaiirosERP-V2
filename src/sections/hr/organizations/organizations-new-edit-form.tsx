        ...
        organizationName: {
          AR:
            currentOrganization?.organizationsTlDTOS?.find((org) => org.langCode === 'AR')
              ?.organizationName ||
            currentOrganization?.organizationName?.AR ||
            '',
          EN:
            currentOrganization?.organizationsTlDTOS?.find((org) => org.langCode === 'EN')
              ?.organizationName ||
            currentOrganization?.organizationName?.EN ||
            '',
        },
        approvalStatus: currentOrganization?.approvalStatus ?? 'DRAFT',
        locationId: locationIds.includes(currentOrganization?.locationId)
          ? currentOrganization?.locationId
          : '',
                ...
                <Box sx={{ gridColumn: { xs: 'span 4', sm: 'span 1', md: 'span 2' } }}>
                  {!organizationLoading && organizations ? (
                    <RHFAutocomplete
                      ...
                      getOptionLabel={(option) => {
                        const selectedOption = !organizationLoading
