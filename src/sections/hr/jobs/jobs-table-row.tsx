...
export default function JobsTableRow({ row, onEditRow, onDeleteRow, deleteLoading }: Props) {
  const { jobName, avatarUrl, active, activeColor, approvalStatus, approvalStatusColor,familyName } = row;
  ...
  return (
    <>
      <TableRow hover>
        ...
        >
          <Avatar alt={jobName} src={`${IMAGE_BASE}/${avatarUrl}`} sx={{ mr: 2 }} />
          <ListItemText primary={jobName} primaryTypographyProps={{ typography: 'body2' }} />
        </TableCell>
        <TableCell sx={{ whiteSpace: 'nowrap' }}> {familyName}</TableCell>
