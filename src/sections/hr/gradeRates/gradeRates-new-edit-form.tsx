...
const GradeRatesNewEditForm = forwardRef<GradeRatesGradeRatesNewEditFormHandle, Props>(
  ({ currentGradeRates, GradeRatesLoading, GRADE_RATE_UNIT, GRADE_RATE_TYPE, operation }, ref) => {
          ...
          <RHFGlobalTextField
                ...
                <Box sx={{ gridColumn: { xs: 'span 4', sm: 'span 1', md: 'span 2' } }}>
                  <RHFAutocomplete
                    ...
                    disabled={isEdit || isPending}
                    options={GRADE_RATE_UNIT.map((option: any) => option.valueCode)}
                    getOptionLabel={(option) => {
                      const selectedOption = GRADE_RATE_UNIT.find(
                ...
                <Box sx={{ gridColumn: { xs: 'span 4', sm: 'span 1', md: 'span 2' } }}>
                  <RHFAutocomplete
                    ...
                    disabled={isEdit || isPending}
                    options={GRADE_RATE_TYPE.map((option: any) => option.valueCode)}
                    getOptionLabel={(option) => {
                      const selectedOption = GRADE_RATE_TYPE.find(
                        (item: any) => item.valueCode === option
                      );
                      return selectedOption ? selectedOption.valueName : '';
                    }}
