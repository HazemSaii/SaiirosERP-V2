...
export default function GradeRatesTableRow({ row, onEditRow, onDeleteRow, deleteLoading }: Props) {
  const {
    gradeRateName,
    avatarUrl,
    gradeRateUnit,
  ...
  return (
    <>
      <TableRow hover>
        ...
        >
          <Avatar alt={gradeRateName} src={`${IMAGE_BASE}/${avatarUrl}`} sx={{ mr: 2 }} />
          <ListItemText primary={gradeRateName} primaryTypographyProps={{ typography: 'body2' }} />
        </TableCell>

        <TableCell sx={{ whiteSpace: 'nowrap' }}> {gradeRateUnit}</TableCell>
        <TableCell sx={{ whiteSpace: 'nowrap' }}> {gradeRateType}</TableCell>
